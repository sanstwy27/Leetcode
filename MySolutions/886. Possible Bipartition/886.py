...
# Copyright   :
# Description : Any %
# Reference   : https://www.cnblogs.com/grandyang/p/10317141.html
# ============================================================================
from typing import collections, List
import queue
class Solution:
    PARTITION1, PARTITION2 = -1, 1
    def possibleBipartition(self, N: int, dislikes: List[List[int]]) -> bool:
        ...
        colors = [0] * (N + 1)
        q = queue.Queue(maxsize = N + 1)
        for i in range(1, N + 1):
            if(colors[i] != 0):
                continue
            else:
                colors[i] = Solution.PARTITION1
                q.put(i)
                while(not q.empty()):
                    cur = q.get()
                    for dis in dic[cur]:
                        if(colors[dis] == colors[cur]):
                            return False
                        elif(colors[dis] == 0):
                            colors[dis] = -colors[cur]
                            q.put(dis)
